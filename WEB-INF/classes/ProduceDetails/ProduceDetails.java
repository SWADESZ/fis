package ProduceDetails;

import java.util.*;
import java.io.*;
import java.sql.*;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

public class ProduceDetails {

    static Connection connection;
    static String query = "";

    public static void deleteProduceDetails(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");
        
        Statement statement = null;

        try {
            
            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

            int i = 0;
            String query = "DELETE FROM produce where pid = '" + request.getParameter("pid") + "'";
            statement = connection.createStatement();
            i = statement.executeUpdate(query);

        } catch (Exception sqle) {
            writer.println("<td align=right>" + sqle + "</td>");
        }

    }

    public static String getPID(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        return request.getParameter("pid");
    }

    public static String getUserView2(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        return request.getParameter("reguserview2");
    }
    
    public static String getPDesc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        return request.getParameter("pdesc");
    }

    public static String getGroup(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        return request.getParameter("group1");
    }
    
    public static ResultSet loadProdDetails(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");
        
        Statement statement = null;

        ResultSet gSoil = null;
        
        java.sql.Connection connection;
        String query = "";

        try {
            
            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");


            int i = 0;
            query = "SELECT * FROM produce where pid = '" + request.getParameter("pid") + "'";
            statement = connection.createStatement();
            gSoil = statement.executeQuery(query);
            
        } catch (Exception sqle) {
            writer.println(sqle);
        }

        return gSoil;
    }

    public static void savePlanting(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        String ip = request.getRemoteAddr();

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

            if (request.getParameter("reguserview3") != null) {

                int added = 0;
                statement = connection.createStatement();
                query = "Update planting Set pDate = '" +  request.getParameter("pdate") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();
                
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set Crop_Type = '" +  request.getParameter("ctype") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();
                
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set Crop_Variety = '" +  request.getParameter("cvariety") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();                
                
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set Crop = '" +  request.getParameter("crops") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();                                                
            
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set AreaPlanted = '" +  request.getParameter("aplanted") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();                                                
                
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set SoilType = '" + request.getParameter("stype") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();                                                                

                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set ExpectedYields = '" +  request.getParameter("eyields") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();                                                
                
                added = 0;
                statement = connection.createStatement();
                query = "Update planting Set EstimatedHarvestingDate = '" +  request.getParameter("ahdate") + "'Where pid='" + request.getParameter("pid") + "'";
                added = statement.executeUpdate(query);
                statement.close();
          
            } else {

                    // Insert Record
                    int added = 0;
                    statement = connection.createStatement();                        
                    query = "INSERT INTO planting(fid, pDate, Crop_Type, Crop_Variety, Crop, AreaPlanted, SoilType, ExpectedYields, EstimatedHarvestingDate) VALUES ('" + request.getParameter("fid") + "','" + request.getParameter("pdate") + "','" + request.getParameter("ctype") + "','" + request.getParameter("cvariety") + "','" + request.getParameter("crops")+ "','" + request.getParameter("aplanted") + "','" + request.getParameter("stype") + "','" + request.getParameter("eyields") + "','" + request.getParameter("ahdate") + "')";
                    added = statement.executeUpdate(query);                                     
            
                    statement.close();
            }
            
        } catch(Exception e) {
            writer.println(e);
        }

    }
    
    public static ResultSet getSuitableSoil(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        ResultSet getSS = null;
        
        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        String ip = request.getRemoteAddr();

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

            int i = 0;
            query = "SELECT * FROM suitablesoils where pid = '" + request.getParameter("pid") + "'order by description";
            statement = connection.createStatement();
            getSS = statement.executeQuery(query);
            boolean proceed = false;  

        } catch(Exception e) {
            writer.println(e);
        }
        
        return getSS;

    }
    
    public static ResultSet getSoils(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        ResultSet getSS = null;
        
        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        String ip = request.getRemoteAddr();

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

            int i = 0;
            query = "SELECT * FROM soils order by description";
            statement = connection.createStatement();
            getSS = statement.executeQuery(query);
            boolean proceed = false;  

        } catch(Exception e) {
            writer.println(e);
        }
        
        return getSS;

    }

    public static ResultSet saveSuitableSoil(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        ResultSet saveSS = null;

        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();

           connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

                    // Insert Record
                    int added = 0;
                    statement = connection.createStatement();                        
                    query = "INSERT INTO suitablesoils(pid, produce, Soil, Description) VALUES ('" + request.getParameter("pid") + "','" + request.getParameter("pdesc") + "','" + request.getParameter("soil") + "','" + request.getParameter("desc") + "')";
                    added = statement.executeUpdate(query);                                     
            
                    statement.close();
    
        } catch (SQLException sqle) {
            writer.println(sqle);
        } catch (Exception e) {
            writer.println(e);        
        }
        
        return saveSS;

    }

    public static ResultSet getProduceMarket(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        ResultSet getPM = null;

        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();

            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

            // Check login details
            int i = 0;
            query = "SELECT * FROM producemarket where pid = '" + request.getParameter("pid") + "' order by produce";
            statement = connection.createStatement();
            getPM = statement.executeQuery(query);
            boolean proceed = false;

        } catch (Exception sqle) {
            writer.println("<td align=right>" + sqle + "</td>");
        }

        return getPM;

    }
    
    public static void saveProduceMarket(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        try {

            Class.forName("com.mysql.jdbc.Driver").newInstance();
        
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");
            
           
                    // Insert Record
                    int added = 0;
                    statement = connection.createStatement();                        
                    query = "INSERT INTO producemarket(pid, Produce, Market, ValidFrom, ValidTo, Price) VALUES ('" + request.getParameter("pid") + "','" + request.getParameter("pdesc") + "','" + request.getParameter("market") + "','" + request.getParameter("vfrom") + "','" + request.getParameter("vto") + "','" + request.getParameter("price") + "')";
                    added = statement.executeUpdate(query);                                     
            
                    statement.close();
            
              
                connection.close();

        } catch (SQLException sqle) {
            writer.println(sqle);
        } catch (Exception e) {
            writer.println(e);        
        }
        
    }
    
    public static ResultSet getSoilByID(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        ResultSet gSID = null;
        
        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");

        try {
            
            Class.forName("com.mysql.jdbc.Driver").newInstance();
        
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");

         // Get record
        int i = 0;
        query = "SELECT * FROM soils where sid = '" + request.getParameter("pid") + "'";
        statement = connection.createStatement();
        gSID = statement.executeQuery(query);
        
        } catch (SQLException sqle) {
            writer.println(sqle);
        } catch (Exception e) {
            writer.println(e);        
        }
        
        return gSID;
        
        
    }
    
    public static void deleteSoil(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
        java.sql.Connection connection;
        Statement statement;
        String query = "";

        PrintWriter writer = response.getWriter();
        response.setContentType("text/html");
        
        try {
            
             Class.forName("com.mysql.jdbc.Driver").newInstance();
        
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/fis", "root", "Makaveli1978");
            
                        // Get record
                        int i = 0;
                        query = "DELETE FROM soils where sid = '" + ProduceDetails.getPID(request,response) + "'";
                        statement = connection.createStatement();
                        i = statement.executeUpdate(query);

                    } catch (Exception sqle) {
                        writer.println("<td align=right>" + sqle + "</td>");
                    }
            
        
        
    }
}